...
class ChessGame:
    def __init__(self, root):
        ...
        self.bot = None
        self.move_history = []  # Lưu lịch sử nước đi dưới dạng SAN để tối ưu update
        ...
        self.history_text = tk.Text(self.history_frame, width=20, height=20, font=("Arial", 10))
    ...
    def update_move_history(self):
        # Tối ưu: Chỉ cập nhật nước đi mới thay vì tính lại toàn bộ lịch sử
        self.history_text.config(state="normal")
        if self.board.move_stack:
            history_board = chess.Board()
            for i, move in enumerate(self.board.move_stack[:len(self.move_history)]):
                history_board.push(move)
            for i, move in enumerate(self.board.move_stack[len(self.move_history):], len(self.move_history)):
                move_san = history_board.san(move)
                if i % 2 == 0:
                    self.move_history.append(f"{i//2 + 1}. {move_san}")
                else:
    ...
    def update_board(self):
        # Tối ưu: Sử dụng list comprehension và tính trước các giá trị hiển thị
        self.canvas.delete("all")
        squares = [(square, chess.square_file(square), chess.square_rank(square)) for square in chess.SQUARES]
        for square, file, rank in squares:
            display_file = (7 - file) if self.board_flipped else file
            display_rank = rank if self.board_flipped else (7 - rank)
    ...
    def player_move(self, event):
        if not self.board.is_game_over() and not self.bot_thinking and self.board.turn == self.player_color:
            ...
            if not (0 <= x <= 7 and 0 <= y <= 7):
                return
            file = (7 - x) if self.board_flipped else x
            rank = y if self.board_flipped else (7 - y)
            ...
            if self.selected_square is None:
                ...
                if (self.board.piece_at(self.selected_square).piece_type == chess.PAWN and
                        (chess.square_rank(square) == 7 if self.board.turn == chess.WHITE else chess.square_rank(square) == 0)):
