...
class ChessGame:
    ...
    def get_opening_move(self):
        if len(self.board.move_stack) >= 10:
            print("Vượt giới hạn khai cuộc, chuyển sang minimax.")
            return None
        ...
        opening_moves = {
            chess.WHITE: ["e2e4", "d2d4", "g1f3", "c2c4"],  # 1. e4, 1. d4, 1. Nf3, 1. c4
            chess.BLACK: ["e7e5", "d7d5", "g8f6", "c7c5"]   # 1...e5, 1...d5, 1...Nf6, 1...c5
        }

        valid_opening_moves = []
        for uci in opening_moves.get(self.board.turn, []):
            move = chess.Move.from_uci(uci)
            if move in legal_moves:
                valid_opening_moves.append(move)

        if not valid_opening_moves:
            print("Không có nước đi khai cuộc phù hợp, chuyển sang minimax.")
            return None

        selected_move = random.choice(valid_opening_moves)
        print(f"Chọn nước đi khai cuộc: {selected_move}")
    ...
    def bot_move_thread(self):
        ...
        best_move = self.get_opening_move() if len(self.board.move_stack) < 10 else None
        if not best_move:
