...
class ChessSearch:
    ...
    def razoring(self, depth, alpha):
        if depth <= 3 and not self.board.is_check():
            eval = self.evaluation.evaluate_board()
            if eval + 150 < alpha:
    ...
    def quiescence_search(self, alpha, beta):
        stand_pat = self.evaluation.evaluate_board()
        if stand_pat >= beta:
            return beta
        if alpha < stand_pat:
            alpha = stand_pat
            ...
            if self.board.gives_check(move):
                ...
                if self.board.is_en_passant(move):
                    ...
                    if captured_piece:
                        ...
                        if see_score > 0 and stand_pat + self.piece_values[captured_piece.piece_type] >= alpha - 200:
                            filtered_moves.append(move)
    ...
    def get_opening_move(self):
        ...
        opening_book_path = "openings.json"
        try:
